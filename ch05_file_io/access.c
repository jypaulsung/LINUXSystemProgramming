...
main(int argc, char *argv[])
{
	...
	while (fread(&rec, sizeof(rec), 1, fp))  { // read records from the file into the "rec" structure
		print_record(&rec);
	}
	...
	fseek(fp, sizeof(rec)*2L, SEEK_SET); // move the file pointer to the 3rd record
	...
	fread(&rec, sizeof(rec), 1, fp);
	strcpy(rec.dept, "Movie"); // change the department of the 3rd record to "Movie"
	fseek(fp, sizeof(rec)*2L, SEEK_SET);
